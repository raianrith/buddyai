...
# User Auth
names = ["Avinash", "Udemy _learner"]
usernames = ["anash", "udmy"]
passwords = ['$2b$12$AOHI5A55LLebqr.1MWOiCuDCWaAS21.G0/Z37O1KFV67uzy8MHxNq',
                '$2b$12$Zc4la61NOgDSlUaqo8KXqebtPG5Zd2yjPZsh49QC2JMuJRm3w04jO']
...
authenticator = stauth.Authenticate(
    credentials, "app_home", "auth", cookie_expiry_days=30)
...
if authentication_status == True:
    ...
    category = st.sidebar.selectbox(
        ...
        options=['Home', 'Study Buddy', 'Job Buddy',
                    'Language Buddy',  'Entertainment & Food Buddy']
    ...
    if category == 'Home':
        ...
        st.markdown("")
        st.subheader(
            "Start using Buddy.AI by browsing through the Menu on your Left...  ⬅️ ")
    ...
    # Recommendation Buddy
        ...
        tab1, tab2, tab3, tab4, tab5 = st.tabs(
            ["Description | ", "Buddy.Music | ", "Buddy.Movie | ", "Buddy.Restaurant | ", "Buddy.Recipe | "])

        with tab1:
            st.markdown(
                "### Hello there! I am your Entertainment & Food Buddy! :)")
            ...
            st.text(
                "Use Entertainment & Food Buddy by browsing through the tabs up top!")
        ...
        with tab2:
            ...
            if option == "Song":
                ...
                song_text = st.text_area(
                    label="Enter name of a song to get suggestions for similar songs: ")
                song_er = st.text_area("By: ")
                song_num = st.number_input(
                    label="How many songs in the playlist would you like?", min_value=1)
                ...
                genre_text = st.text_area(
                    label="Enter a genre to get a playlist matching your genre: ")
                genre_num = st.number_input(
                    label="How many songs in the playlist would you like?", min_value=1)
                ...
                mood_text = st.text_area(
                    label="Enter a mood to get a playlist matching your mood: ")
                mood_num = st.number_input(
                    label="How many songs in the playlist would you like?", min_value=1)
            ...
            lyr_text = st.text_area(
                label="Enter name of a song to get lyrics for: ")
            sing_text = st.text_area(
                label="Enter name of a artist of the song: ")
            ...
            if len(lyr_text) > 1:
                if st.button("Generate Lyrics"):
                    ...
                    with st.spinner('Sit tight! While your buddy gets the job done for you...'):
                        response = openai.Completion.create(
                            engine="text-davinci-003",
                            prompt="Please provide the lyrics in english for the following song: " +
                            str(lyr_text) + " by" + sing_text,
                            max_tokens=516,
                            temperature=0.5)
        ...
        with tab3:
            ...
            if options == "Another Movie":
                ...
                movie_text = st.text_area(
                    label="Enter name of a movie, to get suggestions for similar movies: ")
                movie_num = st.number_input(
                    label="How many movie suggestions would you like?", min_value=1)
                    ...
                    if st.button("Generate Movie Suggestions"):
                        ...
                        with st.spinner('Sit tight! While your buddy gets the job done for you...'):
                            response = openai.Completion.create(
                                ...
                                str(movie_num) + " movies as well as where the movies can be streamed, which are similar to the movie titled: " + movie_text,
                                max_tokens=516,
                                temperature=0.5)
                ...
                mg_text = st.text_area(
                    label="Enter your preferred genre, to get suggestions for movies with similar genre: ")
                mg_num = st.number_input(
                    label="How many movie suggestions would you like?", min_value=1)
                mg_coun = st.selectbox("Which country would you like the movie to be from? ", (
                    'Hollywood', 'Bollywood', 'French', 'Korean', 'International'))
                    ...
                    if st.button("Generate Movie Suggestions"):
                        with st.spinner('Sit tight! While your buddy gets the job done for you...'):
                            ...
                            response = openai.Completion.create(
                                ...
                                prompt="Please provide " +
                                str(mg_num) + " " + mg_coun +
                                " movies as well as where the movies can be streamed for the following genre: " + mg_text,
                                max_tokens=516,
                                temperature=0.5)
                ...
                md_text = st.text_area(
                    label="Enter your preferred mood, to get suggestions for movies with similar mood: ")
                md_num = st.number_input(
                    label="How many movie suggestions would you like?", min_value=1)
                md_coun = st.selectbox("Which country would you like the movie to be from? ", (
                    'Hollywood', 'Bollywood', 'French', 'Korean', 'International'))
                    ...
                    if st.button("Generate Movie Suggestions"):
                        ...
                        with st.spinner('Sit tight! While your buddy gets the job done for you...'):
                            response = openai.Completion.create(
                                ...
                                prompt="Please provide " +
                                str(md_num) + " " + md_coun +
                                "movies as well as where the movies can be streamed for the following mood that I am currently feeling: " + md_text,
                                max_tokens=516,
                                temperature=0.5)
            ...
            rev_text = st.text_area(
                label="Enter name of a movie, to get reviews and ratings: ")
            ...
            if len(rev_text) > 1:
                if st.button("Find Ratings"):
                    with st.spinner('Sit tight! While your buddy gets the job done for you...'):
                        ...
                        response = openai.Completion.create(
                            engine="text-davinci-003",
                            prompt="Find reviews and ratings by movie critiques for the movie: " +
        ...
        with tab4:
            ...
            rstrnt_text = st.text_area(
                label="Enter name of a city (EX: Appleton, Wisconsin), to get suggestions for top restaurants in that city: ")
            rstrnt_num = st.number_input(
                label="How many restaurant suggestions would you like?", min_value=1)
        ...
        with tab5:
            ...
            recipe_text = st.text_area(
                label="Enter name of a dish, to get suggestions for ingridients and recipe of the dish ")
    ...
    # Study Buddy
        ...
        sbtab1, sbtab2, sbtab3, sbtab4, sbtab5 = st.tabs(
            ["Description  | ", "Buddy.Summarize | ", "Buddy.Rephrase | ", "Buddy.PowerPoint | ", "Buddy.Dictionary | "])
        ...
        with sbtab2:
            ...
            sum_text = st.text_area(
                label="Enter a paragraph you would like to summarize: ")
            ...
            if len(sum_text) > 1:
                if st.button("Summarize"):
                    with st.spinner('Sit tight! While your buddy gets the job done for you...'):
                        ...
                        response = openai.Completion.create(
                            engine="text-davinci-003",
                            prompt="Please provide a " + sum_len +
                            " summary for the following paragraph: \n" +
                            str(sum_text),
                            max_tokens=816,
                            temperature=0.5)
        ...
        with sbtab3:
            ...
            par_text = st.text_area(
                label="Enter a sentence/paragraph you would like to rephrase: ")
            ...
            if len(par_text) > 1:
                if st.button("Rephrase!"):
                    with st.spinner('Sit tight! While your buddy gets the job done for you...'):
                        ...
                        response = openai.Completion.create(
                            engine="text-davinci-003",
                            prompt="Please rephrase the following paragraph with a " +
                            str(par_tone) + " tone and add more " + par_add + " relating to the paragraph provided: \n" +
        ...
        with sbtab4:
            ...
            ppt_text = st.text_area(
                label="Enter a paragraph you would like to get powerpoint bulletpoints for: ")
        ...
        with sbtab5:
            ...
            dic_text = st.text_area(
                label="Enter a word you would like to get definition, synonyms, antonyms, and similar words for: ")
            ...
            if len(dic_text) > 1:
                if st.button("Explain!"):
                    with st.spinner('Sit tight! While your buddy gets the job done for you...'):
                        ...
                        response = openai.Completion.create(
                            engine="text-davinci-003",
                            prompt="Please provide definition with example sentences for the following word: " +
        ...
        jbtab1, jbtab2, jbtab3, jbtab4, jbtab5 = st.tabs(
            ["Description  | ", "Buddy.Email | ", "Buddy.Resume | ", "Buddy.Cover_Letter | ", "Buddy.Interview | "])
        ...
        with jbtab2:
            ...
            em_aud = st.text_area(
                label="Who is the email being sent to? [Ex: Dr. Green, Fraternity Club, Students, etc.]")
            em_text = st.text_area(
                label="Enter short description of your email content")
        ...
        with jbtab3:
            ...
            jb_desc = st.text_area(
                label="Copy Paste the Work/Past Experience Section from your resume:")
            jb_aud = st.text_area(
                label="Copy Paste the job description section from a job you are interested in:")
        ...
        with jbtab4:
            ...
            cl_title = st.text_area(
                label="Enter the position you are applying for [Ex: Data Analyst, Masters in Data Science Program, Research Analyst at MIT, etc]: ")
            cl_aud = st.text_area(
                label="Who is the cover letter for? [Data Science Department at Facebook, UW Madison, MIT Research Institute, etc.]")
            cl_text = st.text_area(
                label="Copy Paste Key Points from your Resume: (OPTIONAL)")
            cl_desc = st.text_area(
                label="Copy Paste the description of the Position you are applying for (OPTIONAL)")
        ...
        with jbtab5:
            ...
            iv_title = st.text_area(
                label="Enter the position you are applying for [Ex: Data Analyst, Sales Analyst, Project Manager, etc]: ")
            iv_top = st.selectbox(label="Who topic would you like sample QnA for? ", options=[
                                    "Company Overview", "Education & Qualifications", "Work Experience", "Skills & Abilities", "Career Goals", "Leadership", "Teamwork", "Problem Solving", "Time Management", "Communication"])
        ...
        st.caption("")
        lang_title = st.text_area(
            label="Write something in English that you want translation for ")
        ...
        if len(lang_title) > 1:
            if st.button("Translate!"):
                with st.spinner('Sit tight! While your buddy gets the job done for you...'):
                    ...
                    response = openai.Completion.create(
                        engine="text-davinci-003",
                        prompt="Please translate the the following statement to " +
                        lang_ind + ":\n" + lang_title,
                        max_tokens=816,
                        temperature=0.5)
